...
<h1 class="page-header">Вхлдящие</h1>
			...
			foreach($pkg as $k=>$v) {
				...
				?>
					<tr data-user-id="<?php echo $v->id;?>">
						...
						<td><?php foreach (getPackageNotes($v->id) as $n_user_type=>$n_text) { echo isset($n_text['public']) ? '<strong>'.$n_user_type.'</strong><p>'.$n_text['public'].'</p>' : ''; };?></td>
						...
						<td class="text-center">
							<a href="<?php echo $cfg['options']['siteurl'];?>/dropPkgInfo/<?php echo $v->id;?>/outbox"><i class="fa fa-cogs"></i></a>
						</td>
					</tr>
				...
				?>
					<tr data-user-id="<?php echo $gv->id;?>" class="bg-unread">
						...
						<td class="text-center">
							<a href="<?php echo $cfg['options']['siteurl'];?>/dropPkgInfo/<?php echo $gv->id;?>/outbox"><i class="fa fa-cogs"></i></a>
						</td>
					</tr>					
				<?php
				}
