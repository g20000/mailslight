...
if (!isset($id) || !$id || $id <= 0 || empty($id) || !is_int($id)) {
	exit(json_encode(array('type'=>'error','text'=>'ID пуст')));
}
...
if (!isset($p['email']) || !$p['email'] || empty($p['email'])) {
	$p['email'] = '';
	//exit(json_encode(array('type'=>'error','text'=>'Email is empty')));
}

if (!isset($p['xmpp']) || !$p['xmpp'] || empty($p['xmpp'])) {
	exit(json_encode(array('type'=>'error','text'=>'XMPP пуст')));
}

if ($pass1 != '' || $pass2 != '' ) {
	if ($pass1 !== $pass2) {
		exit(json_encode(array('type'=>'error','text'=>'Пароли должны совпадать')));
	} else {
...
// смотрим можно ли 
if ($user['id']!=$id) {
	exit(json_encode(array('type'=>'error','text'=>'Зарпещено!')));
}
...
if (isset($isnameexist[0])) {
	exit(json_encode(array('type'=>'error','text'=>'Имя уже занято!')));
}
